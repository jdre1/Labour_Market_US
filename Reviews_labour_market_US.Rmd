...
### Walmart average review by City
```{r}
...
data_scientist <- df_all %>%
  select(state, position, reviews) %>%
  filter(position == 'Data Scientist') %>%
  group_by(state) %>%
  arrange(state, desc(state)) %>%
  summarise(avg = sum(reviews))

ggplot(data = data_scientist, aes(x=state, y=avg)) + 
  geom_bar(stat = 'identity') + title('qqqqq')
...
company_data_scientist_mean <- sort(tapply(company_data_scientist$reviews, 
                                           company_data_scientist$company, mean), 
     decreasing = TRUE)
...
title('Top five companies - reviews for pposition Data Scientist')
